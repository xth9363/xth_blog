...
def side_bar(*args, **kwargs):
    # 返回侧边栏数据
    side_data = {
        'new5': Article.objects.order_by('-add_date').all()[:10],
        'read5': Article.objects.order_by('-read_times').all()[:10],
        'tag12': ArticleTag.objects.annotate(num_articles=Count('articles')).filter(num_articles__gt=0).order_by(
            '-num_articles'),
        'article_group': ArticleGroup.objects.order_by('-add_date').all()[:10]
    }
    return side_data
